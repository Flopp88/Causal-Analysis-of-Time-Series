...
def getextendeddelays(gtfile, columns):
    ...
    return extendedgtdelays, readgt, extendedreadgt, pairdelays
...
def evaluate(gtfile, validatedcauses, columns):
    ...
    extendedgtdelays, readgt, extendedreadgt, delays = getextendeddelays(gtfile, columns)
    FP = 0
    FPdirect = 0
    TPdirect = 0
    TP = 0
    ...
    for datafile in datafiles.keys():
        ...
        try:
            with open("results.csv", 'w', newline='') as csvfile:
                ...
                for data in alldelays:
                    list = [str(data[1]) + "," + str(data[0]) + "," + str(alldelays[data])]
        ...
        if evaluation:
            ...
            extendeddelays, readgt, extendedreadgt, delays = getextendeddelays(datafiles[datafile], columns)
        ...
        if args.plot:
            ...
            predicted_graph = plotgraph(stringdatafile, alldelays, columns)
            plt.figure("Grountruth graph")
            real_graph = plotgraph(readgt, delays, columns)
            print(real_graph)

            # sid = SID(real_graph, predicted_graph)
            # print("SID: ", sid)
            plt.show()
...
parser.add_argument('--hidden_layers', type=check_zero_or_positive, default=1,
                    help='Number of hidden layers in the depthwise convolution (default: 1)')
...
parser.add_argument('--plot', action="store_true", default=True, help='Show causal graph (default: True)')
...
if args.ground_truth is not None:
    ...
    main(datafiles, evaluation=True, path=[
        "C:/Users/flori/Downloads/causal_discovery_for_time_series-master/causal_discovery_for_time_series-master/baselines/scripts_python"])
    ...
    ######################################
    print(args.path)
    main(datafiles, evaluation=False, path=[
        "C:/Users/flori/Downloads/causal_discovery_for_time_series-master/causal_discovery_for_time_series-master/baselines/scripts_python"])
    ######################################
